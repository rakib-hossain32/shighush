...
export function StatisticsSection() {
  const stats = [
    { value: "৩৬", label: "মোট অভিযোগ", icon: FileText },
    { value: "১২", label: "এলাকা", icon: MapPinned },
    { value: "০৮", label: "প্রতিষ্ঠান", icon: BarChart3 },
    { value: "৯১%", label: "পরিচয় গোপন", icon: LockKeyhole },
  ];
  return (
    <section
      id="statistics"
      className="border-b-2 border-foreground bg-secondary"
    >
      <div className="mx-auto max-w-7xl px-5 py-12 lg:px-8 lg:py-16">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <p className="font-mono text-[10px] font-bold tracking-[.16em] text-primary">
              ০২ — পরিসংখ্যান
            </p>
          ...
          <p className="max-w-md text-sm leading-7 text-muted-foreground">
            শিবচরের নাগরিক অভিযোগের হালনাগাদ চিত্র — এক নজরে দেখুন।
          </p>
        </div>
        <div className="mt-10 grid grid-cols-2 border-t-2 border-foreground sm:grid-cols-4">
          {stats.map(({ value, label, icon: Icon }) => (
            <div
              key={label}
              className="border-b-2 border-foreground px-4 py-6 sm:border-r-2 sm:last:border-r-0"
            >
              <div className="flex items-center gap-2 text-primary">
                <Icon className="size-4" />
                <span className="font-mono text-[10px] font-bold tracking-[.12em]">
                  {label}
                </span>
              </div>
